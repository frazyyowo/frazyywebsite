// particles.js — black dots, windows only, ignores closed/hidden windows
(() => {
  "use strict";
  ...
  const S = {
    ...
    windowRate: prefersReduced ? 10 : 32,
  ...
  function isVisibleWindow(el) {
    ...
    if (r.right < -80 || r.left > W + 80 || r.bottom < -80 || r.top > H + 80) return false;
  ...
  function perimeterRandom(rect, margin) {
    ...
    if (d < w) {
      return {
        x: rect.left + clamp(d, margin, w - margin),
        y: rect.top,
        nx: 0,
        ny: -1
      };
    ...
    if (d < h) {
      return {
        x: rect.right,
        y: rect.top + clamp(d, margin, h - margin),
        nx: 1,
        ny: 0
      };
    }
  ...
  function rebuildSources() {
    ...
    document.querySelectorAll(".window").forEach(w => {
      if (isVisibleWindow(w)) {
        sources.push({ el: w, rate: S.windowRate });
      }
    });
  }

  const observer = new MutationObserver(rebuildSources);
  ...
  function emitContinuous(src, dt) {
    if (!isVisibleWindow(src.el)) return;
